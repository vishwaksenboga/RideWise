...
async function drawRoute(startCoords, endCoords) {
  ...
  // Use clean circular markers instead of bulky emojis for a professional look
  startMarker = L.circleMarker(startCoords, { color: '#10b981', fillOpacity: 1, radius: 6 }).addTo(map);
  endMarker = L.circleMarker(endCoords, { color: '#ef4444', fillOpacity: 1, radius: 6 }).addTo(map);

  // API Call for actual street coordinates
  ...
  try {
    ...
    routeLayer = L.polyline([startCoords, endCoords], { color: '#3b82f6', weight: 4 }).addTo(map);
  }
}
// ═══════════════════════════════════════════════════════
// THEME
  ...
  input.addEventListener('input', () => {
    ...
    timeout = setTimeout(async () => {
      try {
        ...
        const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&limit=6&addressdetails=1`);
        const data = await response.json();
        ...
        list.querySelectorAll('.autocomplete-item').forEach(el => {
          el.addEventListener('click', () => {
            ...
            window[`_${key}Coords`] = [parseFloat(el.dataset.lat), parseFloat(el.dataset.lon)];
            window[`_${key}Name`] = el.dataset.name;
            list.classList.remove('show');
          });
        });
      } catch (err) {
...
function bookRide(provider) {
  const sLat = window._fromCoords[0], sLon = window._fromCoords[1];
  const dLat = window._toCoords[0], dLon = window._toCoords[1];
  
  let url = '';

  if (provider === 'Uber') {
    url = `uber://?action=setPickup&pickup[latitude]=${sLat}&pickup[longitude]=${sLon}&dropoff[latitude]=${dLat}&dropoff[longitude]=${dLon}`;
  } else if (provider === 'Ola') {
    url = `olacabs://app/launch?lat=${dLat}&lng=${dLon}`;
  } else if (provider === 'Rapido') {
    url = `rapido://booking?pickup_lat=${sLat}&pickup_lng=${sLon}&drop_lat=${dLat}&drop_lng=${dLon}`;
  }

  showToast(`Opening ${provider} app...`, 'success');
  
  // Try to open the app; if it fails, nothing happens (or you could redirect to the web store)
